...
As with the [`useSQLiteContext()`](#usesqlitecontext-hook) hook, you can also integrate the `SQLiteProvider` with [`React.Suspense`](https://react.dev/reference/react/Suspense) to show a fallback component until the database is ready. To enable the integration, pass the `useSuspense` prop to the `SQLiteProvider` component.
...
> **Note:** Due to the nature of async/await, any query that runs while the transaction is active will be included in the transaction. This includes query statements that are outside of the scope function passed to `withTransactionAsync` and may be surprising behavior.

For example, the following test case runs queries inside and outside of a scope function passed to `withTransactionAsync`. However, all of the queries will run within the actual SQL transaction because the second `UPDATE` query runs before the transaction finishes.
...
>     const row = await db.getAsync<{ data: string }>('SELECT data FROM test');
>
...
The [`withTransactionExclusiveAsync`](#withtransactionexclusiveasynctask) function addresses this. Only queries that run within the scope function passed to `withTransactionExclusiveAsync` will run within the actual SQL transaction.
...
The following table summarizes a common API from both [`SQLiteDatabase`](#sqlitedatabase) and [`SQLiteStatement`](#sqlitestatement) classes:
